  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      let q = supabase
        .from("audit_logs")
        .select(
          "id, created_at, user_id, action, entity, entity_id, tenant_id, metadata, tenants(name)",
          {
            count: "exact",
          },
        )
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Histórico de ações e alterações em todas as igrejas.
        </p>
      </div>
      ...
      <Card className="p-4 grid gap-3 sm:grid-cols-3">
        ...
        <Input
          placeholder="Ação (ex: UPDATE:tenants)"
          value={actionFilter}
            ...
            {logs.map((l) => (
              <TableRow key={l.id}>
                ...
                <TableCell className="text-xs">
                  {(l.tenants as { name?: string } | null)?.name ?? "—"}
                </TableCell>
